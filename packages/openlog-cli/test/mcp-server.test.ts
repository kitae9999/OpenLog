...
test("exposes tools according to the active permission profile", async (t) => {
  ...
  assert.ok(
    readOnlyTools.tools.some((tool) => tool.name === "list_workspaces"),
  );
  assert.ok(
    readOnlyTools.tools.some((tool) => tool.name === "start_openlog_session"),
  );
  assert.ok(
    readOnlyTools.tools.some(
      (tool) => tool.name === "get_workspace_agent_guide",
    ),
  );
  assert.ok(
    readOnlyTools.tools.some((tool) => tool.name === "get_workspace_project"),
  );
  assert.ok(
    !readOnlyTools.tools.some((tool) => tool.name === "create_workspace_task"),
  );
  assert.ok(
    !readOnlyTools.tools.some(
  ...
  assert.ok(
    !readOnlyTools.tools.some(
      (tool) => tool.name === "create_workspace_project",
    ),
  );
  assert.ok(
    !readOnlyTools.tools.some(
  ...
  assert.ok(
    !readOnlyTools.tools.some((tool) => tool.name === "delete_workspace_task"),
  );

  const safeWrite = await createSession(t, "safe-write");
  const safeWriteTools = await safeWrite.client.listTools();
  assert.ok(
    safeWriteTools.tools.some((tool) => tool.name === "create_workspace_task"),
  );
  assert.ok(
    safeWriteTools.tools.some(
      (tool) => tool.name === "update_workspace_agent_guide",
    ),
  );
  assert.ok(
  ...
  assert.ok(
    safeWriteTools.tools.some(
      (tool) => tool.name === "create_post_draft_from_output",
    ),
  );
  assert.ok(
    safeWriteTools.tools.some((tool) => tool.name === "create_post_draft"),
  );
  assert.ok(safeWriteTools.tools.some((tool) => tool.name === "publish_post"));
  assert.ok(
    safeWriteTools.tools.some((tool) => tool.name === "unpublish_post"),
  );
  assert.ok(
    !safeWriteTools.tools.some(
      (tool) => tool.name === "publish_workspace_output",
    ),
  );
  assert.ok(
    !safeWriteTools.tools.some((tool) => tool.name === "delete_workspace_task"),
  );
  ...
  assert.ok(
    fullTools.tools.some((tool) => tool.name === "clear_working_brief"),
  );
});
...
test("advertises session instructions and document selection criteria", async (t) => {
  ...
  assert.match(
    ...
    /작업을 시작하기 전에 start_openlog_session/,
  );
  ...
  assert.match(
    session.client.getInstructions() ?? "",
    /발행, Agent Guide 수정, Capture Mode 변경, 삭제/,
  );

  const tools = await session.client.listTools();
  const task = tools.tools.find(
    (tool) => tool.name === "create_workspace_task",
  );
  const log = tools.tools.find((tool) => tool.name === "create_workspace_log");
  const output = tools.tools.find(
    (tool) => tool.name === "create_workspace_output",
  );
  ...
  assert.match(
    ...
    /durable behavior and recording policy/,
  );
  assert.match(updateCapture?.description ?? "", /Capture Mode/);
});

test("uses the remote contract without local paths or image upload", async (t) => {
  const session = await createSession(t, "safe-write", { runtime: "remote" });
  const tools = await session.client.listTools();
  const startSession = tools.tools.find(
    (tool) => tool.name === "start_openlog_session",
  );
  const inputProperties = (
    startSession?.inputSchema as { properties?: Record<string, unknown> }
  )?.properties;

  assert.deepEqual(Object.keys(inputProperties ?? {}), ["projectId"]);
  assert.ok(!tools.tools.some((tool) => tool.name === "upload_post_image"));
  assert.ok(!tools.tools.some((tool) => tool.name === "delete_workspace_task"));
  assert.match(
    session.client.getInstructions() ?? "",
    /경로 없이 start_openlog_session/,
  );
  assert.match(session.client.getInstructions() ?? "", /OAuth 연결 권한/);

  const permissions = await callJson(session.client, "get_mcp_permissions", {});
  assert.equal(permissions.profile, "safe-write");
  assert.match(String(permissions.note), /OAuth connection/);
});
...
test("discovers pathless projects without guessing among multiple choices", async (t) => {
  ...
  session.api.workspaces = [
    workspaceResponse([workspaceProjectResponse("ASK")]),
  ];
  ...
  const selection = await callJson(session.client, "start_openlog_session", {});
...
test("maps workspace read and safe-write tools to their REST endpoints", async (t) => {
  ...
  const cases: Array<{
  ...
  }> = [
    {
      ...
      args: {},
      calls: [{ method: "GET", path: "/workspaces" }],
    },
    {
      tool: "get_workspace",
      args: { workspaceId: 1 },
      calls: [{ method: "GET", path: "/workspaces/1" }],
    },
    {
    ...
    {
      tool: "get_working_brief",
      args: { workspaceId: 1 },
      calls: [{ method: "GET", path: "/workspaces/1/working-brief" }],
    },
    {
      ...
      calls: [
        {
          method: "PUT",
          path: "/workspaces/1/working-brief",
          body: {
            title: "Now",
            prose: "Working",
            taskId: 2,
    ...
    },
    {
      ...
      calls: [
        {
          method: "GET",
          path: "/workspaces/1/activity?from=2026-07-01&to=2026-07-13",
        },
      ],
    },
    {
      ...
      calls: [
        { method: "GET", path: "/workspaces/1/activity/2026-07-13/logs" },
      ],
    },
    {
      ...
      calls: [
        {
          method: "GET",
          path: "/workspaces/1/tasks?status=DOING&cursor=next&size=10",
        },
      ],
    },
    {
      tool: "get_workspace_task",
      args: { workspaceId: 1, taskId: 2 },
      calls: [{ method: "GET", path: "/workspaces/1/tasks/2" }],
    },
    {
    ...
    },
    {
      ...
      calls: [
        {
          method: "GET",
          path: "/workspaces/1/logs?taskId=2&cursor=next&size=20",
        },
      ],
    },
    {
      tool: "get_workspace_log",
      args: { workspaceId: 1, logId: 3 },
      calls: [{ method: "GET", path: "/workspaces/1/logs/3" }],
    },
    {
    ...
    },
    {
      ...
      calls: [
        {
          method: "GET",
          path: "/workspaces/1/todos?from=2026-07-01&to=2026-07-13",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      calls: [
        {
          method: "POST",
          path: "/workspaces/1/todos",
          body: { title: "Review", plannedFor: "2026-07-13", taskId: 2 },
        },
      ],
    },
    {
    ...
    },
    {
      ...
      calls: [
        { method: "GET", path: "/workspaces/1/memories?cursor=next&size=50" },
      ],
    },
    {
      tool: "get_workspace_memory",
      args: { workspaceId: 1, memoryId: 5 },
      calls: [{ method: "GET", path: "/workspaces/1/memories/5" }],
    },
    {
      ...
      calls: [
        {
          method: "POST",
          path: "/workspaces/1/memories",
          body: { title: "Rule", content: "Remember", taskId: 2 },
        },
      ],
    },
    {
    ...
    },
    {
      ...
      calls: [
        {
          method: "PUT",
          path: "/workspaces/1/memories/5",
          body: { title: "Rule", content: "Updated", taskId: null },
        },
      ],
    },
    {
    ...
    {
      tool: "get_workspace_output",
      args: { workspaceId: 1, outputId: 6 },
      calls: [{ method: "GET", path: "/workspaces/1/outputs/6" }],
    },
    {
    ...
    },
    {
      ...
      calls: [
        {
          method: "POST",
          path: "/workspaces/1/task-links",
          body: { fromTaskId: 2, toTaskId: 7, relation: "BLOCKS" },
        },
      ],
    },
    {
      ...
      calls: [
        {
          ...
          path: "/workspaces/1/log-links",
          body: { fromLogId: 3, toLogId: 8, relation: "FIXES" },
        },
      ],
    },
    {
...
test("previews workspace output conversion before the confirmed POST", async (t) => {
  ...
  assert.deepEqual(session.api.calls, [
    { method: "GET", path: "/workspaces/1/outputs/6" },
  ]);

  session.api.calls.length = 0;
  const converted = await callJson(
    session.client,
    "create_post_draft_from_output",
    {
      workspaceId: 1,
...
test("executes full-profile delete tools immediately", async (t) => {
  ...
  const cases: Array<[string, Record<string, unknown>, string]> = [
    ...
    [
      ...
      { workspaceId: 1, taskId: 2 },
      "/workspaces/1/tasks/2",
    ],
    [
      "delete_workspace_log",
      { workspaceId: 1, logId: 3 },
      "/workspaces/1/logs/3",
    ],
    [
      "delete_workspace_todo",
      { workspaceId: 1, todoId: 4 },
      "/workspaces/1/todos/4",
    ],
    [
      "delete_workspace_memory",
      { workspaceId: 1, memoryId: 5 },
      "/workspaces/1/memories/5",
    ],
    [
      "delete_workspace_output",
      { workspaceId: 1, outputId: 6 },
      "/workspaces/1/outputs/6",
    ],
    [
      "delete_workspace_task_link",
      { workspaceId: 1, taskLinkId: 7 },
      "/workspaces/1/task-links/7",
    ],
    [
      "delete_workspace_log_link",
      { workspaceId: 1, logLinkId: 8 },
      "/workspaces/1/log-links/8",
    ],
    [
      "delete_workspace_cross_link",
      { workspaceId: 1, crossLinkId: 9 },
      "/workspaces/1/cross-links/9",
    ],
  ];
...
test("normalizes a missing working brief and returns other API errors", async (t) => {
  const session = await createSession(t, "read-only");
  session.api.failures.set(
    "/workspaces/1/working-brief",
    new ApiError(404, "test", "missing"),
  );
...
test("validates IDs, dates, ranges, and enums before calling the API", async (t) => {
  ...
  const invalidCalls = [
    { name: "get_workspace", arguments: { workspaceId: 0 } },
    {
      ...
      arguments: { workspaceId: 1, title: "Task", status: "ACTIVE" },
    },
    {
      name: "list_workspace_todos",
      arguments: { workspaceId: 1, from: "2026-07-13" },
    },
    {
      name: "list_workspace_todos",
      arguments: { workspaceId: 1, plannedFor: "2026-02-30" },
    },
    {
      name: "get_workspace_activity",
      arguments: { workspaceId: 1, from: "2026-07-13", to: "2025-07-13" },
    },
  ];
...
async function createSession(
  ...
  const created = await createOpenLogMcpServer({
    ...
    permissions,
    createAuthenticatedClient: async () => api as unknown as OpenLogApiClient,
    readAuth: async () => null,
