locals {
  ...
  alert_rules = {
    ...
    mcp_metrics_missing = {
      name           = "OpenLog MCP metrics missing"
      expr           = "max((max_over_time(up{job=\"openlog-mcp\"}[3m]) < bool 1) or absent_over_time(up{job=\"openlog-mcp\"}[3m]))"
      evaluator_type = "gt"
      threshold      = 0
      pending_period = "0s"
      no_data_state  = "Alerting"
      severity       = "critical"
      summary        = "Alloy cannot scrape the remote MCP metrics endpoint."
    }
    mcp_remote_disabled = {
      name           = "OpenLog remote MCP disabled"
      expr           = "min(openlog_mcp_enabled{job=\"openlog-mcp\"})"
      evaluator_type = "lt"
      threshold      = 1
      pending_period = "2m"
      no_data_state  = "Alerting"
      severity       = "critical"
      summary        = "The remote MCP public route has been disabled for two minutes."
    }
    mcp_5xx = {
      name           = "OpenLog MCP internal error rate high"
      expr           = <<-EOT
        max(
          (sum(increase(openlog_mcp_responses_total{job="openlog-mcp",status_class="5xx"}[5m])) >= bool 3)
          or
          (
            (sum(increase(openlog_mcp_requests_total{job="openlog-mcp"}[5m])) >= bool 20)
            *
            (
              sum(increase(openlog_mcp_responses_total{job="openlog-mcp",status_class="5xx"}[5m]))
              /
              clamp_min(sum(increase(openlog_mcp_requests_total{job="openlog-mcp"}[5m])), 1)
              > bool 0.05
            )
          )
        )
      ...
      no_data_state  = "OK"
      severity       = "warning"
      summary        = "Remote MCP produced at least three 5xx responses or exceeded a 5% internal error rate."
    }
    mcp_p95 = {
      name           = "OpenLog MCP p95 latency high"
      expr           = <<-EOT
        histogram_quantile(
          0.95,
          sum by (le) (rate(openlog_mcp_request_duration_seconds_bucket{job="openlog-mcp"}[5m]))
        )
        *
        (sum(increase(openlog_mcp_requests_total{job="openlog-mcp"}[5m])) >= bool 20)
      EOT
      ...
      no_data_state  = "OK"
      severity       = "warning"
      summary        = "Remote MCP p95 latency exceeded three seconds with at least 20 requests."
    }
    mcp_token_exchange_failures = {
      name           = "OpenLog MCP token exchange failures"
      expr           = "sum(increase(openlog_mcp_token_exchange_duration_seconds_count{job=\"openlog-mcp\",outcome=\"failure\"}[5m]))"
      evaluator_type = "gt"
      threshold      = 2
      pending_period = "0s"
      no_data_state  = "OK"
      severity       = "critical"
      summary        = "Remote MCP failed to exchange three or more internal API tokens in five minutes."
    }
    disk_critical = {
